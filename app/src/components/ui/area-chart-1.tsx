...
const LEGEND_ITEMS = [
  { name: 'Composite Tension', color: '#DC2626' },
  { name: 'Political Volatility', color: '#EAB308' },
  { name: 'Security Incidents', color: '#F97316' },
];
...
// Converted to STI OSINT related metrics, keeping the same shape
...
const METRICS_DATA: MetricInfo[] = [
  {
    ...
    label: 'Mean Escalation Time',
    tooltip: 'Mean Time to Escalation',
    value: '6 Hours',
  ...
  },
  {
    ...
    label: 'Mobilization Delay',
    tooltip: 'Time before mass unrest',
    value: '4 Hours',
    TrendIcon: UpTrendIcon,
    trendBaseColor: '#E84045',
    trendStrokeColor: '#F08083',
  ...
  },
  {
    ...
    label: 'Tension Diffusion Rate',
    tooltip: 'De-escalation coefficient',
    value: '10%',
...
const IncidentReportCard: React.FC = () => {
  return (
    <>
      <div className="flex flex-col pt-4 pb-4 bg-[#111113] rounded-2xl shadow-[11px_21px_3px_rgba(0,0,0,0.06),14px_27px_7px_rgba(0,0,0,0.10),19px_38px_14px_rgba(0,0,0,0.13),27px_54px_27px_rgba(0,0,0,0.16),39px_78px_50px_rgba(0,0,0,0.20),55px_110px_86px_rgba(0,0,0,0.26)] w-full overflow-hidden transition-colors duration-300">
        <h3 className="text-xl text-left p-7 pt-2 pb-4 font-bold text-white transition-colors duration-300">
          STI Trend Matrix
        </h3>
        <div className="flex justify-between w-full px-8 mb-4">
          {LEGEND_ITEMS.map((item) => (
            <div key={item.name} className="flex gap-2 items-center">
              ...
              <span className="text-white/50 font-medium text-[10px] uppercase tracking-wider transition-colors duration-300">{item.name}</span>
            </div>
          ))}
        </div>
        <div className="reaviz-chart-container h-[200px] px-4"> 
          <AreaChart
            height={200}
              ...
              <LinearXAxis
                ...
                tickSeries={
                  <LinearXAxisTickSeries
                    label={
                      <LinearXAxisTickLabel
                        format={(v: any) => new Date(v).toLocaleDateString('en-US', { month: 'numeric', day: 'numeric' })}
            ...
            series={
              <AreaSeries
                ...
                colorScheme={['#DC2626', '#EAB308', '#F97316']} 
              />
            }
            gridlines={<GridlineSeries line={<Gridline strokeColor="var(--reaviz-gridline-stroke)" />} />}
          />
        </div>
        <div className="flex flex-col px-8 pt-8 font-mono divide-y divide-white/5 transition-colors duration-300">
          {METRICS_DATA.map((metric) => (
            ...
            >
              <div className="flex flex-row gap-2 items-center text-xs font-semibold uppercase tracking-wider w-1/2 text-white/50 transition-colors duration-300">
                <metric.Icon />
              ...
              <div className="flex gap-2 w-1/2 justify-end items-center">
                <span className="font-semibold text-lg text-white transition-colors duration-300">{metric.value}</span>
